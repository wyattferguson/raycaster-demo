...
def main():
    ...
    while True:
                ...
                if mapGrid[y][x] != 0:
                    pygame.draw.rect(SCREEN,
                                     ...
                                     ((x * (map_size / mapBoundX) + WIDTH) - map_size,
                                      y * (map_size / mapBoundY) + HEIGHT - map_size,
                                         (map_size / mapBoundX),
                                         (map_size / mapBoundY)))
        ...
        pygame.draw.line(
            SCREEN, (255, 0, 127),
            pos_on_map,
            ((dirY + posY + planeY) * (map_size / mapBoundY) + WIDTH - map_size, (dirX + posX + planeX) *
             (map_size / mapBoundX) + HEIGHT - map_size))
        pygame.draw.line(
            SCREEN, (255, 0, 127),
            pos_on_map,
            ((dirY + posY - planeY) * (map_size / mapBoundY) + WIDTH - map_size, (dirX + posX - planeX) *
             (map_size / mapBoundY) + HEIGHT - map_size))
        pygame.draw.line(
            SCREEN, (255, 0, 127),
            ((dirY + posY + planeY) * (map_size / mapBoundY) + WIDTH - map_size, (dirX + posX + planeX) *
             (map_size / mapBoundX) + HEIGHT - map_size),
